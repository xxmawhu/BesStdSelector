...
#   File Name     : BeSelectorSecondaryPionSelector.cxx
...
#include "BeSelector/Selector/BeSelectorSecondaryPionSelector.h"

BeSelectorSecondaryPionSelector::BeSelectorSecondaryPionSelector() {
    m_pidtype = 0;
    ...
    m_propMgr.declareProperty("RxyCut", m_VrCut = 10.0);
    m_propMgr.declareProperty("Vz0Cut", m_VzCut = 20.0);
    ...
    jobSvc->setMyProperties("BeSelectorSecondaryPionSelector", &m_propMgr);
}

bool BeSelectorSecondaryPionSelector::operator()(
    CDChargedPion& aSecondaryPion) {

    aSecondaryPion.setUserTag(1);
    EvtRecTrack* recTrk = const_cast<EvtRecTrack*>(aSecondaryPion.track());
    ...
    HepLorentzVector p4pi = aSecondaryPion.p4();
    ...
    if (!(pid->probPion() > 0 && pid->probPion() > pid->probProton() &&
        ...
        aSecondaryPion.setUserTag(2);

    if (aSecondaryPion.userTag() == 1)
        return true;
    else
        return false;
}
void BeSelectorSecondaryPionSelector::setpidtype(bool dopid) {
    m_DoPID = dopid;
}
